...
let chat = db.chats[anu.id] || {}

if (anu.action == 'add') {
console.log('Ada yang masuk gc om:V')
await makeWelcome(hisoka, num, metadata.subject).then(async(res) => {   
         if(res){
         var button = [
             { 
              buttonId: `ahsudahlah`, 
 ...
 console.log('Ada yang keluar gc om:V')
        await makeGodbyee(hisoka, num, metadata.subject).then(async(res) => {   
          if(res){
          var button = [
             { 
              buttonId: `ahsudahlah`, 
