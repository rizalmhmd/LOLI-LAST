...
const makeMessagesSocket = (config) => {
    ...
    const fetchPrivacySettings = async (force = false) => {
        if (!privacySettings || force) {
            ...
            const nodes = (0, WABinary_1.getBinaryNodeChildren)(result, 'category');
            privacySettings = nodes.reduce((dict, { attrs }) => {
    ...
    const getUSyncDevices = async (jids, ignoreZeroDevices) => {
        ...
        const { device } = (0, WABinary_1.jidDecode)(authState.creds.me.id);
        const extracted = (0, Utils_1.extractDeviceJids)(result, device, ignoreZeroDevices);
        const deviceMap = {};
    ...
    const relayMessage = async (jid, message, { messageId: msgId, additionalAttributes, cachedGroupMetadata }) => {
        ...
        if (isGroup) {
            const { ciphertext, senderKeyDistributionMessageKey } = await (0, Utils_1.encryptSenderKeyMsgSignalProto)(destinationJid, encodedMsg, authState.creds.me.id, authState);
            ...
            const { user: meUser } = (0, WABinary_1.jidDecode)(authState.creds.me.id);
            ...
            participants.push(await createParticipantNode((0, WABinary_1.jidEncode)(user, 's.whatsapp.net'), encodedMsg));
            participants.push(await createParticipantNode((0, WABinary_1.jidEncode)(meUser, 's.whatsapp.net'), encodedMeMsg));
