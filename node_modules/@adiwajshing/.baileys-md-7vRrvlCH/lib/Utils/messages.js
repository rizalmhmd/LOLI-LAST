...
const prepareWAMessageMedia = async (message, options) => {
    ...
    if (cacheableKey) {
        ...
        if (mediaBuff) {
            return Types_1.WAProto.Message.decode(mediaBuff);
        }
    }
    ...
    const { mediaKey, encBodyPath, bodyPath, fileEncSha256, fileSha256, fileLength, didSaveToTmpPath } = await (0, messages_media_1.encryptedStream)(uploadData.media, mediaType, requiresOriginalForSomeProcessing);
    ...
    try {
        ...
        (_a = options.logger) === null || _a === void 0 ? void 0 : _a.info({ trace: error.stack }, 'failed to obtain extra info');
    }
    const { mediaUrl } = await options.upload((0, fs_1.createReadStream)(encBodyPath), { fileEncSha256B64, mediaType, timeoutMs: options.mediaUploadTimeoutMs });
    // remove tmp files
    await Promise.all([
...
const generateWAMessage = async (jid, content, options) => ((0, exports.generateWAMessageFromContent)(jid, await (0, exports.generateWAMessageContent)(content, options), options));
